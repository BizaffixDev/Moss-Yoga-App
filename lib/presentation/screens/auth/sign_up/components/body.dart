...
import '../../login/components/or_login_with_text.dart';
...
class _BodyState extends ConsumerState<Body> {
  ...
  void initState() {
    ...
    print('The Current Screen is SIGN UP STUDENT');
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ref.listen<AuthStates>(authNotifyProvider, (previous, screenState) async {
      if (screenState is GoogleSignUpErrorState) {
        ...
        showLoading(context);
        // setState(() {});
      } else if (screenState is SignUpStudentLoadingState) {
        debugPrint('Loading');
        showLoading(context);
      } else if (screenState is SignupSuccessfulStudentState) {
        ...
        GoRouter.of(context)
            .push('${PagePath.verifyEmailStudent}?email=$email');
        ...
        GoRouter.of(context).push(PagePath.level);
        // GoRouter.of(context).go(PagePath.verifyEmail);
        dismissLoading(context);
      }
    });
      ...
      if (screenState is AuthErrorState) {
        if (screenState.errorType == ErrorType.unauthorized) {
          UIFeedback.showSnackBar(context, screenState.error.toString(),
              height: 140);
          dismissLoading(context);
        } else if (screenState.errorType == ErrorType.inline) {
          print("This is the error thats not shwoing: ${screenState.error}");
          UIFeedback.showSnackBar(context, screenState.error.toString());
          dismissLoading(context);
        } else {
          print("This is the error thats weirld showing: ${screenState.error}");
          UIFeedback.showSnackBar(context, screenState.error.toString());
          dismissLoading(context);
        }
      } else if (screenState is SignUpStudentLoadingState) {
        ...
        //  stateType: StateType.initial,height: 280);
        final email = _emailController.text.trim();
        GoRouter.of(context).push('${PagePath.verifyEmailStudent}?email=$email');
        // GoRouter.of(context).go(PagePath.verifyEmail);
        dismissLoading(context);
                            ...
                            CustomButton(
                              ...
                              onTap: () async {
                                if (_usernameController.text.isEmpty ||
                                  ...
                                  ///Remove these and uncomment the ones below
                                  // final email = _emailController.text.trim();
                                  // GoRouter.of(context)
                                  //     .push('${PagePath.verifyEmail}?email=$email');
