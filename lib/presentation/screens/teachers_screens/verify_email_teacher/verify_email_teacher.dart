...
class VerifyEmailTeacherScreen extends ConsumerStatefulWidget {
  String email;

  VerifyEmailTeacherScreen({Key? key, required this.email}) : super(key: key);

  @override
  ConsumerState<VerifyEmailTeacherScreen> createState() => _VerifyEmailTeacherScreenState();
}

class _VerifyEmailTeacherScreenState extends ConsumerState<VerifyEmailTeacherScreen> {
  int start = 120;
  bool wait = false;
  ...
  _sendOtp() {
    ...
    ref
        ...
        .otpVerificationTeacher(context, email: email.toString(), code: code);
  }

  @override
  Widget build(BuildContext context) {
    ref.listen<AuthStates>(authNotifyProvider, (previous, screenState) async {
      if (screenState is OTPVerificationTeacherErrorState) {
        setState(() {
        ...
        if (screenState.errorType == ErrorType.unauthorized) {
          ...
          UIFeedback.showSnackBar(context, screenState.error.toString(),height: 200);
          dismissLoading(context);
        } else {
      ...
      }else if (screenState is OTPEmailVerificationTeacherLoadingState) {
        debugPrint('Loading');
        showLoading(context);
      } else if (screenState is OTPVerificationTeacherSuccessfulState) {
        otpController.clear(); // Clear the otpController text field
        print(
            'Inside the success state of OTPVerificationStudentSuccessfulState');
        // dismissLoading(context);
        context.push(PagePath.teacherRegProcess);
